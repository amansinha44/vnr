  ...
  const industries = [
    {
      ...
      side: "left",
      theme: "from-[#f0f7ff]/60 to-[#f8f5ff]/60"
    },
    {
      id: "education",
      icon: "🎓",
    ...
    <div className="relative w-full min-h-screen font-sans selection:bg-[#5a67d8]/30 selection:text-[#1d1d1f] pb-20">
      ...
      <div className="relative z-10 pt-32 space-y-24">
        ...
        <section className="w-full max-w-[1200px] mx-auto px-4 md:px-8 text-center relative">
          <span className="inline-block py-1.5 px-4 rounded-xl bg-white/60 backdrop-blur-md border border-white text-[#7e22ce] text-[10px] font-black tracking-[0.3em] uppercase mb-6 shadow-sm">
          ...
          <h1 className="text-5xl md:text-7xl font-black text-[#1a1a2e] leading-[1.1] tracking-tight mb-6">
            ...
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#4338ca] via-[#7e22ce] to-[#be185d]">Follows Life.</span>
          </h1>
        ...
        <section className="relative w-full max-w-[1200px] mx-auto px-4 md:px-8">
          ...
          <div className="hidden md:block absolute left-1/2 top-0 bottom-0 w-[2px] bg-gradient-to-b from-[#4338ca] via-[#7e22ce] to-transparent -translate-x-1/2 opacity-30"></div>

          <div className="space-y-24">
            {industries.map((item) => (
              <div key={item.id} className="relative flex flex-col md:flex-row items-center group">
                
                <div className="hidden md:block absolute left-1/2 w-5 h-5 rounded-full bg-white border-4 border-[#7e22ce] -translate-x-1/2 z-20 shadow-[0_0_15px_rgba(126,34,206,0.5)] group-hover:scale-125 transition-transform duration-300"></div>
                  ...
                  <div className={`p-6 md:p-10 rounded-md border border-white/80 shadow-[0_8px_32px_rgba(0,0,0,0.04)] bg-white/50 backdrop-blur-2xl bg-gradient-to-br ${item.theme} relative overflow-hidden group-hover:shadow-md transition-all duration-300`}>
                    ...
                    <div className="relative z-10 space-y-5">
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        <span className="text-[#7e22ce] font-bold text-[10px] md:text-xs tracking-[0.2em] uppercase bg-white/80 px-3 py-1 rounded-md shadow-sm">{item.year}</span>
                        {item.badge && (
                          <span className="bg-gradient-to-r from-[#4338ca] to-[#be185d] text-white text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-widest whitespace-nowrap shadow-sm">{item.badge}</span>
                        )}
                      </div>

                      <h2 className="text-2xl md:text-4xl font-black text-[#1a1a2e] leading-[1.1] tracking-tight">{item.headline}</h2>
                      <p className="text-sm md:text-base text-slate-700 font-medium leading-relaxed italic border-l-4 border-[#7e22ce]/40 pl-4 bg-white/30 py-2 rounded-r-md">
                        "{item.content}"
                      </p>
                      
                      <div className="pt-2 md:pt-4">
                        <p className="text-xs md:text-sm font-black text-[#7e22ce] uppercase tracking-[0.2em] mb-3">We Help With:</p>
                      ...
                      {item.quote && (
                        <p className="pt-3 text-base md:text-lg font-bold text-transparent bg-clip-text bg-gradient-to-r from-[#4338ca] via-[#7e22ce] to-[#be185d] italic drop-shadow-sm">
                          "{item.quote}"
                        </p>
                      )}
                    </div>
        ...
        {/* --- SECTION: THE GRAND FINALE (NEON PINK THEME) --- */}
